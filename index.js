...
if (!TOKEN) {
  console.error("❌ لم يتم العثور على TOKEN في Variables في Railway.");
  process.exit(1);
}
...
function isStaff(member) {
  return STAFF_ROLES.some(
    roleName => hasRole(member, roleName)
  );
}
...
async function registerCommands() {
  ...
  try {
    ...
    // نأخذ Application ID تلقائيًا من البوت
    const applicationId =
      client.user.id;

    console.log(
      "🔄 جاري تسجيل أوامر Boti..."
    );
    ...
    if (interaction.isButton()) {
      ...
      if (
        ...
        if (existingTicket) {
          ...
          return interaction.reply({
            content:
              `❌ لديك تكت مفتوحة بالفعل: ${existingTicket}`,
            ephemeral: true
          });
        ...
        // إعطاء رتب الإدارة صلاحية التكت
        for (
          const roleName of STAFF_ROLES
        ) {
        ...
        const embed =
          new EmbedBuilder()
            ...
            .setDescription(
              ...
              `🛡️ الإدارة ستقوم بمساعدتك قريباً.`
      ...
      if (
        ...
        return interaction.reply({
          ...
          content:
            `⚠️ تم استخدام Claim مرة ثانية في نفس التكت.\n\n` +
            `❌ **-50 نقطة**\n` +
            `📊 نقاطك الآن: **${points}**`
